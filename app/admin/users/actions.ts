    ...
    try {
        ...
        if (validatedFields.studentId) {
            const existingStudentId = await db
                .select()
                .from(users)
                .where(sql`LOWER(${users.studentId}) = LOWER(${validatedFields.studentId})`)
                .limit(1);
    ...
    try {
        ...
        const [usersResult, totalCountResult] = await Promise.all([
            db
                ...
                .groupBy(users.id, users.name, users.email, users.username, users.studentId, users.image, users.emailVerified)
