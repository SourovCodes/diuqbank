...
async function QuestionDetailContent({ params }: QuestionDetailPageProps) {
  ...
  const fileName = `${question.courseName}-${question.departmentShortName}-${question.examTypeName}.pdf`.replace(/[^a-zA-Z0-9.-]/g, '_');
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        ...
        <PDFViewer
          pdfUrl={pdfUrl}
          fileName={fileName}
          fileSize={question.pdfFileSizeInBytes}
        />
      </div>
    </div>
  );
...
export default async function QuestionDetailPage(props: QuestionDetailPageProps) {
  return (
