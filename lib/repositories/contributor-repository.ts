...
export class ContributorRepository {
  ...
  async findPublicContributors(page: number = 1, pageSize: number = 12) {
    ...
    const contributors = (
      ...
      totalViews:
        typeof r.totalViews === "number"
          ? (r.totalViews as number)
          : parseInt(String(r.totalViews ?? 0), 10),
      latestQuestionDate: r.latestQuestionDate
        ? new Date(r.latestQuestionDate as string | Date)
        : null,
  ...
  async findPublicContributorByUsername(
    username: string
    ...
    const baseContributor = {
      ...
      questionCount:
        typeof agg.questionCount === "number"
          ? (agg.questionCount as number)
          : parseInt(String(agg.questionCount ?? 0), 10),
      totalViews:
        typeof agg.totalViews === "number"
          ? (agg.totalViews as number)
          : parseInt(String(agg.totalViews ?? 0), 10),
    ...
    const [deptRows] = await db.execute(sql`
        ...
        `);
    const departments = (
      deptRows as unknown as Array<Record<string, unknown>>
    ).map((r) => ({
    ...
    const [courseRows] = await db.execute(sql`
        ...
        `);
    const courses = (
      courseRows as unknown as Array<Record<string, unknown>>
    ).map((r) => ({
      name: r.name === null ? null : String(r.name),
      departmentName:
        r.departmentName === null ? null : String(r.departmentName),
      count:
        typeof r.count === "number"
          ? (r.count as number)
          : parseInt(String(r.count ?? 0), 10),
    ...
    const [examRows] = await db.execute(sql`
        ...
        `);
    const examTypes = (
      examRows as unknown as Array<Record<string, unknown>>
    ).map((r) => ({
      name: r.name === null ? null : String(r.name),
      count:
        typeof r.count === "number"
  ...
  async findContributorQuestions(
    ...
    const [qRows] = await db.execute(sql`
            ...
            LIMIT ${pageSize} OFFSET ${skip}
        `);
    const questions = (qRows as unknown as Array<Record<string, unknown>>).map(
      (r) => ({
        id: Number(r.id),
        ...
        departmentName:
          r.departmentName === null ? null : String(r.departmentName),
        departmentShortName:
          r.departmentShortName === null ? null : String(r.departmentShortName),
