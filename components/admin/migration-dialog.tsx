...
interface MigrationDialogProps {
    ...
    migrateAction: (fromId: string, toId: string) => Promise<{ success: boolean; error?: string; migratedCount?: number }>;
    ...
    const handleMigrate = async () => {
        ...
        try {
            ...
            if (response.success) {
                ...
                toast.success(
                    `Successfully migrated ${migratedCount} question${migratedCount !== 1 ? 's' : ''} to the selected ${entityType.toLowerCase()}`
                );
                setIsOpen(false);
                setSelectedTargetId("");
                onMigrationComplete?.();
            } else {
                toast.error(response.error || "Migration failed");
            }
        } catch (error) {
            ...
            <DialogContent className="sm:max-w-md">
                    ...
                    <div className="space-y-2">
                        ...
                        <Select value={selectedTargetId} onValueChange={setSelectedTargetId}>
                            <SelectTrigger id="target-select">
                                <SelectValue placeholder={`Select target ${entityType.toLowerCase()}`} />
                            </SelectTrigger>
                            <SelectContent>
                ...
                <DialogFooter className="sm:justify-start">
                    ...
                    <Button
                        ...
                        disabled={!selectedTargetId || isLoading || filteredEntities.length === 0}
                    >
