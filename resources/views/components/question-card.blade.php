...
<div class="relative overflow-hidden border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 hover:border-blue-400 dark:hover:border-blue-600 transition-all duration-200 shadow-sm hover:shadow-md h-full rounded-xl group cursor-pointer" onclick="window.location.href='{{ url('/questions/'.$question->id) }}'">
    {{-- Invisible overlay for clicking anywhere on the card --}}
    
    <div class="px-4 py-4 relative z-10 flex flex-col h-full">
            ...
            <div class="flex items-center gap-1 flex-shrink-0">
                ...
                @auth
                    @if(auth()->id() === $question->user_id)
                        <div class="relative z-20 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <a href="{{ route('questions.edit', $question) }}" 
                                class="inline-flex items-center justify-center h-7 w-7 rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors"
                                title="Edit question"
                                onclick="event.stopPropagation()">
                                <x-lucide-pencil class="h-3 w-3" />
                            </a>
                            <form method="POST" action="{{ route('questions.destroy', $question) }}" class="inline" onsubmit="return confirmDeleteCard(event, '{{ addslashes($question->course->name ?? 'this question') }}')">
                                @csrf
                                @method('DELETE')
                                <button type="submit" 
                                    class="inline-flex items-center justify-center h-7 w-7 rounded-md border border-red-300 dark:border-red-600 bg-white dark:bg-slate-800 text-red-700 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
        ...
        <div class="absolute bottom-4 right-4 h-6 w-6 rounded-full bg-blue-100 dark:bg-blue-900/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
...
<script>
function confirmDeleteCard(event, courseName) {
