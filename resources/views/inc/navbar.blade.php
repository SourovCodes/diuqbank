...
<div class="bg-indigo-900 text-center py-2 lg:px-4">
	<a href="https://www.facebook.com/groups/1298831488164950" target="_blank" class="p-2 bg-indigo-800 items-center text-indigo-100 leading-none lg:rounded-full flex lg:inline-flex" role="alert">
	  <span class="flex rounded-full bg-indigo-500 uppercase px-2 py-1 text-xs font-bold mr-3">New</span>
	  <span class="font-semibold mr-2 text-left flex-auto">Join our facebook group</span>
		...
		<div x-cloak x-show="offCanvasMenu" x-transition:enter="transition ease-in-out duration-300 transform"
			...
			<div class="border-t border-gray-200 dark:border-gray-700 py-6 px-4 space-y-4">
				...
				@auth
					...
					<x-nav-link :href="route('contributors.show', auth()->user()->username)"
					            :active="request()->routeIs('contributors.show')">
						<x-svg.user-circle class="inline w-4 h-4"/> {{ __('Profile') }}
					</x-nav-link>

					<x-nav-link :href="route('my-account.questions.create')"
					            :active="request()->routeIs('my-account.questions.create')">
